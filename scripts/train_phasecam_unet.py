"""Train the direct U-Net PhaseCam baseline."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the reduced PhaseCam U-Net baseline.")
    parser.add_argument("--config", default="configs/training/unet512.yaml")
...
def main() -> int:
    ...
    model = build_model("unet", config["model"], forward_model)
    ...
    best_model = load_checkpoint_model(checkpoint_dir / "best.pt", "unet", config["model"], forward_model, device)
